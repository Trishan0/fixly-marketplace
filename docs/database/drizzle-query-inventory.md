...
Repository instrumentation attaches an operation name such as `marketplace.accept-proposal` and the request ID to slow-query and failure logs without recording SQL text or bind values. Every multi-record repository read has an explicit `LIMIT`; the `bounded-read-models.test.js` gate prevents a new list operation from omitting one. `npm run typecheck:persistence` strictly checks the database infrastructure, environment, request context, and Drizzle schema while the remaining CommonJS runtime conversion is being completed. The Phase 11 release runbook requires retained `EXPLAIN (ANALYZE, BUFFERS)` evidence for each registry item marked as performance-sensitive before production traffic is enabled.
