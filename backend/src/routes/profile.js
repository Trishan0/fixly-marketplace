...
router.put('/ai-matching-opt-in', verifyToken, requireRole('worker'), async (req, res) => { if (typeof req.body.opt_in !== 'boolean') return res.status(400).json({ error: 'opt_in must be a boolean' }); try { const result = await repository.setAiMatchingOptIn(req.user.id, req.body.opt_in); res.json({ message: 'AI matching preference updated', ai_matching_opt_in: result.ai_matching_opt_in }); } catch (error) { respond(error, res); } });
