...
router.get('/assigned', verifyToken, requireRole('worker'), async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT j.*, c.name as category_name, c.icon as category_icon,
              u.full_name as customer_name
  ...
  try {
    ...
    if (req.user.role === 'worker') {
      ...
      if (!['in_progress', 'completed'].includes(status)) {
        return res.status(400).json({ error: 'Workers can only set in_progress or completed' });
      }
    }
    ...
    if (req.user.role === 'customer') {
      ...
      if (status !== 'cancelled') {
        return res.status(400).json({ error: 'Customers can only cancel jobs' });
      }
    ...
    if (!allowed.includes(status)) {
      return res.status(400).json({ error: `Invalid transition from ${job.status} to ${status}` });
    }
    ...
    if (status === 'in_progress') {
      await createNotification(job.customer_id, 'job_started', 'Job Started', `Worker has started working on: ${job.title}`, { job_id: job.id });
    } else if (status === 'completed') {
      await createNotification(job.customer_id, 'job_completed', 'Job Completed', `Worker has completed: ${job.title}`, { job_id: job.id });
    }
...
router.delete('/:id', verifyToken, requireRole('customer'), async (req, res) => {
  try {
    const jobResult = await pool.query('SELECT * FROM jobs WHERE id = $1 AND customer_id = $2', [req.params.id, req.user.id]);
    if (!jobResult.rows[0]) return res.status(404).json({ error: 'Job not found' });
    
    const job = jobResult.rows[0];
    if (['in_progress', 'completed', 'payment_recorded', 'reviewed'].includes(job.status)) {
      return res.status(400).json({ error: 'Cannot delete job in current state' });
    }

    await pool.query('UPDATE jobs SET status = $1, is_active = false WHERE id = $2', ['cancelled', req.params.id]);
...
router.post('/:id/photos', verifyToken, requireRole('customer'), upload.array('photos', 6), async (req, res) => {
  const jobResult = await pool.query('SELECT * FROM jobs WHERE id = $1 AND customer_id = $2', [req.params.id, req.user.id]);
  if (!jobResult.rows[0]) return res.status(404).json({ error: 'Job not found' });
