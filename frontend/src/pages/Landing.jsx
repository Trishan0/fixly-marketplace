...
function HeroVisual({ user }) {
  ...
  return (
    <div className="relative">
      <div className="absolute inset-0 rounded-[2rem] bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.16),transparent_38%),radial-gradient(circle_at_bottom_right,rgba(14,165,233,0.12),transparent_34%)]" />
          ...
          <div className="rounded-[1.5rem] bg-slate-950 p-5 text-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-300">Browse, match, hire, complete, and review without losing context.</p>
              </div>
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#f8fbff_0%,#eff6ff_100%)]">
      <nav className="border-b border-slate-100 bg-white/90 px-6 py-5 backdrop-blur md:px-12">
          ...
          <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <div className="inline-flex items-center gap-2 rounded-full border border-sky-100 bg-white px-4 py-1.5 shadow-sm">
                ...
                <span className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-700">{hero.eyebrow}</span>
              </div>
              ...
              <div className="mt-10 flex flex-col gap-3 pb-6 sm:flex-row sm:flex-wrap">
                <Link to={hero.primary.to} className="fixly-btn-primary text-base px-8 py-3.5 rounded-2xl">
                  {hero.primary.label}
                </Link>
                <Link to={hero.secondary.to} className="fixly-btn-secondary text-base px-8 py-3.5 rounded-2xl">
                  {hero.secondary.label}
                </Link>
                <Link to={hero.tertiary.to} className="inline-flex items-center gap-1 px-2 py-3 text-base font-semibold text-sky-700 transition hover:text-sky-800">
              ...
              <motion.div
                ...
                className="mt-8 flex flex-wrap gap-8 border-t border-slate-200 pt-8"
              >
      ...
      <section className="mx-auto max-w-6xl px-6 py-20 md:px-12">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {categories.map(({ icon: Icon, name, color }) => (
            <Link key={name} to={`/workers?category=${name}`} className="fixly-card group flex flex-col items-center gap-3 p-6 hover:shadow-md transition-shadow">
              <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${color}`}>
                <Icon className="w-6 h-6" />
              </div>
              <span className="text-sm font-semibold text-slate-800 transition-colors group-hover:text-sky-600">{name}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {!user && (
        <section className="bg-sky-600 py-20 text-center text-white">
          <div className="mx-auto max-w-2xl px-6">
            ...
            <div className="flex justify-center gap-3">
              <Link to="/auth?tab=register" className="rounded-2xl bg-white px-8 py-3 font-bold text-sky-600 transition-all hover:bg-sky-50">
                Create Account
              </Link>
              <Link to="/workers" className="rounded-2xl bg-sky-700 px-8 py-3 font-bold text-white transition-all hover:bg-sky-800">
                Browse Workers
              </Link>
            </div>
          </div>
