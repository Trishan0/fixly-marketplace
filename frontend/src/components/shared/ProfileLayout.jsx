...
export function PublicPageChrome({ crumbLabel, crumbTo, currentLabel }) {
  return (
    <nav className="border-b border-white/70 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-6 py-4">
        ...
        {crumbLabel && (
          <>
            ...
            {crumbTo ? (
              <Link to={crumbTo} className="text-sm text-slate-500 hover:text-slate-900">{crumbLabel}</Link>
            ) : (
...
export function ProfileHeroCard({ avatarName, avatarSrc, header, summary, stats, asideTitle, asideContent, children }) {
  return (
    <Card className="overflow-hidden border-white/90 bg-white/95 shadow-[0_24px_70px_rgba(15,23,42,0.07)]">
      <div className="grid gap-6 p-6 lg:p-8 xl:grid-cols-[1.4fr_0.8fr]">
        <div className="space-y-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
            <Avatar
              ...
              src={avatarSrc}
              size="xl"
              className="ring-4 ring-sky-50 shadow-[0_12px_30px_rgba(15,23,42,0.10)]"
            />
        ...
        <div className="rounded-[1.75rem] border border-slate-100 bg-slate-50/90 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]">
...
export function ProfileSectionCard({ title, meta, children, className }) {
  return (
    <Card className={cn('border-white/90 bg-white/95 p-6 shadow-[0_16px_44px_rgba(15,23,42,0.05)]', className)}>
