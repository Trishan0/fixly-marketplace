...
export function Avatar({ name, src, size = 'md', className }) {
  ...
  return (
    <div className={cn('rounded-xl bg-sky-100 text-sky-600 dark:bg-sky-950/50 dark:text-sky-300 font-bold flex items-center justify-center flex-shrink-0 overflow-hidden', sizes[size], className)}>
  ...
  return (
    <div className={cn('fixly-card p-5', className)}>
      <div className="flex items-start justify-between">
        ...
        <div className={cn('flex h-10 w-10 items-center justify-center rounded-2xl', colors[color])}>
...
export function Select({ label, error, className, children, ...props }) {
  return (
    <div className="space-y-1.5">
      {label && <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">{label}</label>}
      <select className={cn('fixly-input bg-white dark:bg-slate-900', error && 'border-red-400', className)} {...props}>
        {children}
      </select>
      {error && <p className="text-xs text-red-500">{error}</p>}
    </div>
...
export function Spinner({ className }) {
  return <div className={cn('h-5 w-5 animate-spin rounded-full border-2 border-slate-200 border-t-sky-600 dark:border-slate-700', className)} />
}
...
export function Modal({ open, onClose, title, children, width = 'max-w-lg' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={cn('relative w-full overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900', width)}>
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800">
          <h2 className="text-lg font-bold text-slate-900">{title}</h2>
          <button onClick={onClose} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-white">
...
export function StarRating({ rating, size = 'sm' }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <svg key={i} className={cn(sizes[size], i <= Math.round(rating) ? 'text-amber-400' : 'text-slate-200 dark:text-slate-700')} fill="currentColor" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      ))}
      {rating > 0 && <span className="ml-1 text-xs text-slate-500">{Number(rating).toFixed(1)}</span>}
    </div>
  )
}
